...
TEST(AUTOVALORES, N2){
        ...
        bool e = r1.size() == a.size() &&
            std::all_of(r1.begin(), r1.end(), [&](const d_type& m) {
...
TEST(AUTOVETORES, N2){
        matrix m({2,2}, {2,1,2,1});
        // talvez trocar para um vector
        std::vector<matrix> r1 = {matrix({2,1}, {-1,1}), matrix({2,1}, {1,1}),};
        std::vector<matrix> a = m.autovetores();
        // comparacao independete da ordem
        bool e = r1.size() == a.size() &&
        std::all_of(r1.begin(), r1.end(), [&](const matrix& m) {
            return std::find(a.begin(), a.end(), m) != a.end();
             });
